...
function findUserForGoods(goods) {
  if (!goods) return null
  return mock.users.find((item) => Number(item.id) === Number(goods.sellerId))
    || findUserByUsername(goods.username)
    || findUserByNickname(goods.sellerName)
    || null
}

function findUserForMessage(message, conversation) {
  if (!message) return null
  const current = getCurrentUser()
  if (message.from === 'me') return current
  return findUserByUsername(message.senderUsername)
    || findUserByNickname(message.senderName)
    || findUserByUsername(conversation && conversation.peerUsername)
    || findUserByNickname(conversation && conversation.peer)
    || null
}

function getDisplayImage(item) {
  const images = (item && item.images) || []
  const imageObjects = (item && item.imageObjects) || []
  const fromObject = imageObjects.map((image) => image && (image.url || image.localUrl || image.uploadedUrl)).find(Boolean)
  return images.find(Boolean) || fromObject || (item && item.image) || ''
}

function decorateGoods(goods) {
  if (!goods) return goods
  const seller = findUserForGoods(goods) || {}
  const sellerName = seller.nickname || goods.sellerName || '同校用户'
  const username = seller.username || goods.username || 'user'
  const image = getDisplayImage(goods)
  return Object.assign({}, goods, {
    sellerId: seller.id || goods.sellerId,
    sellerName,
    username,
    sellerAvatar: seller.avatar || goods.sellerAvatar || '',
    sellerInitial: getInitial(sellerName),
    image,
    images: (goods.images && goods.images.length ? goods.images : (image ? [image] : [])),
    imageObjects: goods.imageObjects || []
  })
}

function decorateService(item) {
  if (!item) return item
  const owner = item.type === 'errand' && item.status === 'waiting_accept'
    ? findUserByUsername(item.username)
    : mock.users.find((user) => user.nickname === item.provider || user.username === item.riderUsername || user.username === item.username)
  const relatedOrder = item.type === 'errand'
    ? mock.orders.find((order) => order.itemType === 'errand' && Number(order.itemId) === Number(item.id))
    : null
  const name = owner ? owner.nickname : (item.provider || '同校用户')
  return Object.assign({}, item, {
    provider: name,
    username: owner ? owner.username : (item.username || 'user'),
    ownerAvatar: owner ? (owner.avatar || '') : '',
    orderSn: item.orderSn || (relatedOrder && relatedOrder.orderSn) || '',
    images: item.images || [],
    imageObjects: item.imageObjects || []
  })
}

function decorateConversation(conversation) {
  if (!conversation) return conversation
  const peerUser = findUserByUsername(conversation.peerUsername) || findUserByNickname(conversation.peer) || {}
  const peer = peerUser.nickname || conversation.peer || '交易对象'
  const peerUsername = peerUser.username || conversation.peerUsername || 'user'
  return Object.assign({}, conversation, {
    peer,
    peerUsername,
    peerAvatar: peerUser.avatar || conversation.peerAvatar || '',
    peerInitial: getInitial(peer),
    messages: (conversation.messages || []).map((message) => {
      const sender = findUserForMessage(message, conversation) || {}
      const senderName = sender.nickname || message.senderName || (message.from === 'me' ? '我' : peer)
      const senderUsername = sender.username || message.senderUsername || (message.from === 'me' ? 'campus_user' : peerUsername)
...
function decorateOrder(order) {
  ...
  const counterpartyUser = isWaitingErrandPeer ? null : (findUserByUsername(counterpartyUsername) || findUserByNickname(counterpartyName))
  const counterpartyLabel = order.counterpartyLabel || (order.itemType === 'service' ? '服务者' : order.itemType === 'errand' ? '骑手' : '卖家')
  const counterpartyLine = order.counterpartyLine || (order.isPublication
    ? `${counterpartyLabel}：等待交易`
    : isWaitingErrandPeer ? `${counterpartyLabel}：等待接单` : `${counterpartyLabel}：${counterpartyName} @${counterpartyUsername}`)
  return Object.assign({}, order, {
    ...
    counterpartyAvatar: counterpartyUser ? (counterpartyUser.avatar || '') : '',
    counterpartyLabel,
    counterpartyLine,
    canChat: !isWaitingErrandPeer && !order.isPublication,
...
function buildPublishRecords() {
  const current = getCurrentUser()
  const goodsRecords = mock.goods
    .filter((item) => Number(item.sellerId) === Number(current.id))
    .map((item) => ({
      ...
      createdTs: item.createdTs || Number(item.id) || Date.now(),
      progressText: item.auditNote || (item.status === 'on_sale' ? '商品已发布，等待同学下单或联系。' : '发布记录已保存。'),
      isPublication: true,
      events: [item.status === 'on_sale' ? '发布成功' : ORDER_STATUS_TEXT[item.status] || '发布记录'],
      timeline: [
        {
          title: item.status === 'on_sale' ? '发布成功' : ORDER_STATUS_TEXT[item.status] || '发布记录',
          desc: item.auditNote || item.desc || '发布信息已同步到我的发布。',
          time: item.createdAt || '刚刚',
          done: true
        }
      ]
    }))
  const serviceRecords = mock.services
    .filter((item) => item.type !== 'errand' && item.username === current.username)
    .map((item) => ({
      ...
      createdAt: item.createdAt || '',
      createdTs: item.createdTs || Number(item.id) || Date.now(),
      progressText: item.status === 'on_sale' ? '服务已发布，等待同学预约或咨询。' : '发布记录已保存。',
      isPublication: true,
      events: [item.status === 'on_sale' ? '发布成功' : ORDER_STATUS_TEXT[item.status] || '发布记录'],
      timeline: [
        {
          title: item.status === 'on_sale' ? '发布成功' : ORDER_STATUS_TEXT[item.status] || '发布记录',
          desc: item.desc || '服务信息已同步到我的发布。',
          time: item.createdAt || '刚刚',
          done: true
        }
      ]
    }))
  return goodsRecords.concat(serviceRecords)
}
...
function ensureConversation(data) {
  ...
  if (data.peerAvatar && !conversation.peerAvatar) conversation.peerAvatar = data.peerAvatar
  if (data.peerName && conversation.peer !== data.peerName) conversation.peer = data.peerName
  if (data.peerUsername && conversation.peerUsername !== data.peerUsername) conversation.peerUsername = data.peerUsername
  return conversation
}
...
function getPublicUser(userId) {
  ...
  return {
    ...
    reviews,
    goods: mock.goods.filter((item) => item.sellerId === user.id && item.status === 'on_sale').map(decorateGoods),
    services: activeServices.map(decorateService)
  }
}
...
function decorateServiceDetail(id) {
  ...
  return Object.assign({}, item, {
    ...
    ownerAvatar: publicUser.user.avatar || '',
    images: item.images || [],
    imageObjects: item.imageObjects || [],
...
function api(options) {
  ...
  if (url === '/api/goods/list') return setCache(options, ok({ list: mock.goods.map(decorateGoods), categories: mock.categories }))
  if (url.indexOf('/api/goods/detail') === 0) {
    const item = mock.goods.find((g) => g.id === Number(data.id)) || mock.goods[0]
    return setCache(options, ok(decorateGoods(item)))
  }
  if (url === '/api/goods/save') {
    ...
    mock.goods.unshift({
      ...
      status,
      sellerId: current.id || 1,
      sellerName: current.nickname || '校园同学',
      username: current.username || 'campus_user',
      sellerAvatar: current.avatar || '',
      image: images[0] || getDisplayImage({ imageObjects }),
      images,
      imageObjects,
      desc: data.desc,
  ...
  if (url === '/api/goods/favorites') return setCache(options, ok({ list: mock.goods.filter((item) => item.favorite).map(decorateGoods) }))
  if (url === '/api/goods/mine') {
    const current = getCurrentUser()
    return setCache(options, ok({ list: mock.goods.filter((item) => Number(item.sellerId) === Number(current.id)).map(decorateGoods) }))
  }
  ...
  if (url === '/api/order/list') {
    const list = mock.orders.concat(buildPublishRecords()).sort((a, b) => {
      const bt = Number(b.createdTs || 0)
      const at = Number(a.createdTs || 0)
  ...
  if (url === '/api/service/list') return setCache(options, ok({ list: mock.services.map(decorateService) }))
  ...
  if (url === '/api/chat/list') return setCache(options, ok({ list: mock.conversations.map(decorateConversation) }))
  if (url === '/api/chat/messages') {
    const conversation = ensureConversation(data)
    const decorated = decorateConversation(conversation)
    return setCache(options, ok({ conversation: decorated, list: decorated.messages }))
  }
  if (url === '/api/chat/send') {
    ...
    conversation.messages.push({
      id: Date.now(),
      from: 'me',
      senderName: current.nickname || '校园同学',
