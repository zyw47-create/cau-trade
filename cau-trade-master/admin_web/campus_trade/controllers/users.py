...
def api_rider_earnings():
    try:
        ...
        return api_error(exc)
    return api_ok({"total": summary["total"], "amount": summary["amount"], "pendingAmount": summary["pendingAmount"], "acceptedCount": summary["acceptedCount"], "list": [row_to_api(row) for row in summary["list"]], "withdraws": [row_to_api(row) for row in summary["withdraws"]]})
