...
BEGIN
  ...
  DECLARE v_status VARCHAR(20);
  DECLARE v_existing_order_sn VARCHAR(64);
  DECLARE v_not_found TINYINT DEFAULT 0;
  ...
  SELECT provider_id, price, title, description, status
    INTO v_provider_id, v_price, v_title, v_description, v_status
  FROM services
  ...
  IF v_status <> 'on_sale' THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'service is not available';
  END IF;

  IF v_provider_id = p_buyer_id THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'buyer cannot order own service';
  END IF;

  SET v_not_found = 0;
  SELECT order_sn INTO v_existing_order_sn
  FROM orders
  ...
  IF v_not_found = 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'service order already exists';
  END IF;

  INSERT INTO orders
    (order_sn, buyer_id, seller_id, item_type, item_id, item_snapshot, amount, status, remark)
  VALUES
    (p_order_sn, p_buyer_id, v_provider_id, 'service', p_service_id,
     JSON_OBJECT('title', v_title, 'price', v_price),
     v_price, 'unpaid', COALESCE(p_remark, v_description));

  INSERT INTO order_events
    (order_sn, from_status, to_status, operator_id, event_type, note)
  VALUES
    (p_order_sn, NULL, 'unpaid', p_buyer_id, 'service_order', '预约校园服务');
...
BEGIN
  ...
  SELECT buyer_id, amount, status, item_type, item_id
    INTO v_buyer_id, v_amount, v_status, v_item_type, v_item_id
  FROM orders
  WHERE order_sn = p_order_sn
  FOR UPDATE;
...
BEGIN
  ...
  SELECT buyer_id, seller_id, item_type, item_id, status
    INTO v_buyer_id, v_seller_id, v_item_type, v_item_id, v_status
  FROM orders
  WHERE order_sn = p_order_sn
  FOR UPDATE;

  IF p_operator_id NOT IN (v_buyer_id, v_seller_id) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'operator is not order participant';
  END IF;

  IF v_status <> 'unpaid' THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'only unpaid order can be cancelled directly';
  END IF;
  ...
  INSERT INTO order_events
    (order_sn, from_status, to_status, operator_id, event_type, note)
  VALUES
    (p_order_sn, v_status, 'cancelled', p_operator_id, 'cancel', COALESCE(p_reason, '用户取消订单'));
...
BEGIN
  ...
  INSERT INTO order_events
    (order_sn, from_status, to_status, operator_id, event_type, note)
  VALUES
    (p_order_sn, 'confirmed', 'shipped', p_seller_id, 'ship', '卖家/服务者发货或开始履约');
...
BEGIN
  ...
  IF v_status NOT IN ('paid','confirmed','shipped','completed') THEN
