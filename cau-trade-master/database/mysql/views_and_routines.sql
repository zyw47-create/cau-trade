...
DROP PROCEDURE IF EXISTS sp_wallet_reconcile;
...
SELECT
  ...
  CASE
    WHEN o.status = 'unpaid' AND f.id IS NULL THEN 0
    WHEN f.id IS NULL THEN 1
    WHEN f.amount <> o.amount THEN 1
    WHEN o.status IN ('paid','confirmed','shipped','disputed') AND f.status <> 'frozen' THEN 1
    WHEN o.status = 'refunding' AND f.status <> 'refunding' THEN 1
    WHEN o.status = 'completed' AND f.status <> 'settled' THEN 1
    WHEN o.status IN ('refunded','cancelled') AND f.status <> 'refunded' THEN 1
    ELSE 0
  END AS is_abnormal
FROM orders o
LEFT JOIN order_funds f ON f.order_sn = o.order_sn
...
CREATE PROCEDURE sp_wallet_reconcile(IN p_stat_date DATE)
BEGIN
  DECLARE v_checked_count INT DEFAULT 0;
  DECLARE v_abnormal_count INT DEFAULT 0;

  SELECT COUNT(*), COALESCE(SUM(is_abnormal), 0)
    INTO v_checked_count, v_abnormal_count
  FROM v_wallet_reconcile_source v
  JOIN orders o ON o.order_sn = v.order_sn
  WHERE DATE(COALESCE(o.paid_at, o.created_at)) <= p_stat_date;

  INSERT INTO job_logs
    (job_name, status, scanned_count, success_count, fail_count, message, started_at, finished_at)
  VALUES
    ('sp_wallet_reconcile', IF(v_abnormal_count = 0, 'success', 'partial'),
     v_checked_count, v_checked_count - v_abnormal_count, v_abnormal_count,
     CONCAT('reconcile_date=', p_stat_date, ', abnormal_count=', v_abnormal_count),
     NOW(), NOW());
...
CREATE TRIGGER trg_errand_orders_after_update
AFTER UPDATE ON errand_orders
FOR EACH ROW
BEGIN
  IF NOT (OLD.status <=> NEW.status) THEN
    ...
    VALUES
      (NEW.id, NEW.rider_id, 'status_change', OLD.status, NEW.status, 'status changed by database trigger');
  END IF;
END$$
