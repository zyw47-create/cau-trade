...
本项目是一个面向校园场景的二手闲置、校园服务与跑腿任务综合交易平台。系统包含微信小程序端、业务 API 后端、MySQL 数据库设计与运维脚本、Python Flask 管理后台，以及需求、概要设计、详细设计、数据设计、数据流图和 ER 图等课程交付文档。
...
> Legacy note: `backend/server.js` is kept only for old compatibility experiments. The mini-program and database acceptance should use the Flask API in `admin_web/app.py` on `http://127.0.0.1:5000`.
...
.
├── miniprogram/                         # 微信小程序源码
|   |-- app.js                           # Global config, default remote API, baseUrl and verifyBaseUrl
...
│   │   └── admin/                       # 小程序内管理演示页
│   └── utils/
|       |-- api.js                       # API routing, cache, audit fallback, and business flows
│       ├── mock.js                      # 演示数据
│       └── store.js                     # 登录态、实名态、当前用户状态
- Node.js is not required for the current Flask + MySQL mini-program backend; only use it when explicitly testing the legacy compatibility service.
│   ├── server.js                        # 商品、服务、跑腿、订单、聊天、邮箱验证码等 API
...
│   ├── verify.sql                       # 数据库验收查询
│   ├── redis_keys.md                    # Redis 热点缓存、分布式锁与限流键名规范
...
| ????? API | Python?Flask?PyMySQL??? RESTful JSON ??????????? MySQL |
| ???? | Flask ?? HMAC-SHA256 JWT???????? Bearer Token |
| ???? | Flask `/api/user/email-code` ? `/api/user/verify`??? SMTP???????????????? |
| ???? | SMTP??? QQ ???163 ???????? SMTP ?? |
| ??? | MySQL 8.0???????????????????Event????? |
| ORM ??? | SQLAlchemy ?????Alembic ??????????????? MySQL ???? |
| ??????? | Redis ??/???????? Redis ??? MySQL ???????? |
| ???? | Python?Flask?PyMySQL?????? MySQL ?????????????????? |
| ???? | PowerShell?mysqldump?SHA-256 ?? |
| ???? | draw.io?docx?txt????? |
...
```powershell
cd D:\大三下\软件工程\cau-trade-master
powershell -NoProfile -ExecutionPolicy Bypass -File .\start-all.ps1
```

???????

- Flask 业务 API 与管理后台：`http://127.0.0.1:5000`
- ????? API?`http://127.0.0.1:5000/api/user/email-code`
...
- `apiMode: 'remote'` 表示核心业务流程请求本地 Flask REST API。
- `baseUrl` 默认指向 `http://127.0.0.1:5000`，用于商品、服务、订单、聊天和跑腿等核心业务。
- `verifyBaseUrl` ???? `http://127.0.0.1:5000`?????????????? Flask REST API ???
- `miniprogram/utils/api.js` ???? API ?????????????????????????????? Flask/MySQL?
- ???? `/api/auth/login` ? Flask ?? HMAC-SHA256 JWT???????? `Authorization: Bearer <token>`?
...
## 启动 Flask 业务 API 与管理后台

Flask 目录：[admin_web](admin_web)

Flask 服务同时提供管理后台页面和小程序核心 REST API：
...
小程序业务 API 使用同一 Flask 服务的 `/api/*` 接口。

敏感写接口包括下单、支付、取消、确认收货、退款、跑腿抢单和聊天发送，均需要请求头：

```text
Authorization: Bearer <JWT>
```

Flask ????? HMAC-SHA256 JWT ??????
...
The mini-program fallback layer and Flask API keep blocked-keyword checks for classroom demonstration, for example:
...
```text
POST http://127.0.0.1:5000/api/user/email-code net::ERR_CONNECTION_REFUSED
```

处理：

1. 确认后端已启动。
2. 访问 `http://127.0.0.1:5000/api/status`。
3. ????? `5000`?
4. 微信开发者工具勾选“不校验合法域名、TLS 版本以及 HTTPS 证书”。

### 微信开发者工具提示合法域名问题

??????? `http://127.0.0.1:5000`??? HTTPS ?????????????????????????? HTTPS ???????????? request ?????
...
- 重新编译小程序。
- 清除微信开发者工具缓存。
- Keep `apiMode: 'remote'`, then start Flask API and MySQL before classroom demo.
...
Before production launch, finish production payment, HTTPS request domain, real identity provider, logging/monitoring, and security hardening.
