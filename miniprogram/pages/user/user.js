...
Page({
  ...
  onLoad(query) {
    ...
    this.username = query.username ? decodeURIComponent(query.username) : ''
    this.preview = store.takePendingUserPreview()
    const samePreview = this.preview && (
      (this.userId && String(this.preview.id) === String(this.userId)) ||
      (!this.userId && this.username && String(this.preview.username || '') === String(this.username))
    )
    if (samePreview) {
      wx.nextTick(() => {
        this.applyUserData({
  ...
  loadUser() {
    ...
    api({ url: '/api/user/public', data: { id: this.userId, username: this.username } }).then((res) => {
