...
function buildTrade(conversation) {
  ...
  const orderSn = conversation.orderSn || conversation.order_sn || ''
  return {
...
Page({
  ...
  loadMessages() {
    ...
    api({
      ...
      this.setData({
        ...
        peerId: conversation.peerId || conversation.peer_id || this.data.peerId,
        messages,
  ...
  openPeerProfile() {
    ...
    const peerId = conversation.peerId || conversation.peer_id || this.data.peerId || ''
    const username = conversation.peerUsername || this.data.peerUsername || ''
    if (!peerId && !username) return
    store.setPendingUserPreview({
      id: peerId,
      nickname: conversation.peer || this.data.peerName,
      username,
      avatar: conversation.peerAvatar || this.data.peerAvatar || ''
    })
    if (peerId) {
      wx.navigateTo({ url: `/pages/user/user?id=${peerId}` })
      return
    }
    wx.navigateTo({ url: `/pages/user/user?username=${encodeURIComponent(username)}` })
  },
