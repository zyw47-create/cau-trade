...
BasePage({
  ...
  loadEvidence(e) {
    const refundId = e.currentTarget.dataset.id
    if (!refundId || this.data.evidenceLoading) return
    this.setData({ evidenceLoading: true })
    api({ url: `/api/admin/refunds/${refundId}/evidence` }).then((res) => {
      const detail = res.data || {}
      const messages = (detail.messages || []).map((item) => {
        const hash = item.contentHash || item.content_hash || ''
        const hashStatus = item.hashStatus || item.hash_status || ''
        return Object.assign({}, item, {
          ...
          hashText: hashStatus === 'ok' ? '校验通过' : '需复核',
          statusText: item.status === 'normal' ? '正常' : (item.status || '未知')
        })
      })
      this.setData({
        evidenceDetail: Object.assign({}, detail, { messages }),
        evidenceLoading: false
      })
    }).catch(() => {
  ...
  closeEvidence() {
    this.setData({ evidenceDetail: null })
  },
