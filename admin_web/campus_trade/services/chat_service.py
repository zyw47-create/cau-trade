...
def find_chat_target(data: dict, current_user_id: int) -> dict | None:
    ...
    if not business_type or not business_id:
        return None
    explicit_peer_id = _to_int(data.get("peerId") or data.get("receiverId"))
    peer_id = chat_repository.business_owner(business_type, business_id)
    if not peer_id or peer_id == current_user_id:
        peer_id = explicit_peer_id
    if not peer_id:
        peer = chat_repository.user_by_username(str(data.get("peerUsername") or data.get("peer_username") or "").strip())
        if peer and int(peer["id"]) != int(current_user_id):
            peer_id = int(peer["id"])
    if not peer_id:
        return None
...
def get_messages(user_id: int, conversation_id: str, datetime_filter, item_type_text) -> dict:
    ...
    conv["orderSn"] = conversation.get("order_sn") or ""
    conv["peerId"] = conversation.get("peer_id")
