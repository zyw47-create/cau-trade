...
def is_verified_for_business(user_id: int) -> bool:
    user = user_repository.get_profile(user_id)
    if not user:
        return False
    if user.get("role") == "admin":
        return True
    return bool(int(user.get("is_verified") or 0))
...
def create_comment(user_id: int, data: dict) -> dict:
    ...
    if not is_verified_for_business(user_id):
        raise UserError("real-name verification is required before commenting")
