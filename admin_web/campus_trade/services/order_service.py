...
def create_goods_order(user_id: int, data: dict) -> dict:
    if not user_repository.is_user_verified(user_id):
        raise OrderError("real-name verification is required before ordering")
...
def pay_order(user_id: int, data: dict) -> dict:
    if not user_repository.is_user_verified(user_id):
        raise OrderError("real-name verification is required before payment")
...
def take_errand(user_id: int, data: dict) -> dict:
    if not user_repository.is_user_verified(user_id):
        raise OrderError("real-name verification is required before taking errands")
