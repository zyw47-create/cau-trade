...
@bp.route("/api/admin/refunds/<int:refund_id>/evidence")
@bp.route("/v1/api/admin/refunds/<int:refund_id>/evidence")
@require_admin_api
def api_admin_refund_evidence(refund_id: int):
    try:
        detail = admin_query_service.fetch_refund_evidence(refund_id)
    except DatabaseError as exc:
        return api_error(exc)
    if not detail:
        return api_error("refund evidence not found", code=404, status=404)
    return api_ok(row_to_api(detail))
