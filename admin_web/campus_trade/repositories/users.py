...
def user_id_by_username(username: str) -> int | None:
    ...
    if not text:
        return None
    with session_scope() as session:
        user_id = session.scalar(select(User.id).where(User.username == text).limit(1))
...
def create_comment_with_audit_event(
    ...
    with session_scope() as session:
        ...
        if target_user:
            before_score = int(target_user.credit_score or 100)
            if score >= 5:
                change_value = 1
                reason_detail = "五星好评，信用分增加"
            elif score <= 2:
                change_value = -2
                reason_detail = "低分评价，信用分扣减"
            else:
                change_value = 0
                reason_detail = "评价记录，信用分保持"
            next_score = max(0, min(100, before_score + change_value))
            actual_change = next_score - before_score
            target_user.credit_score = next_score
            target_user.updated_at = now
            session.add(
                CreditLog(
                    user_id=target_user_id,
                    change_value=actual_change,
                    reason_type="comment_received",
