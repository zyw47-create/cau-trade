...
def user_by_username(username: str) -> dict | None:
    ...
    if not text:
        return None
    with session_scope() as session:
        user = session.execute(select(User).where(User.username == text).limit(1)).scalar_one_or_none()
        if not user:
            return None
        return {
            "id": int(user.id),
            ...
            "creditScore": user.credit_score or 0,
        ...
        rows = session.execute(
            select(
                ...
                (
                    ...
                    .where(
                        Order.item_type == Conversation.business_type,
                        Order.item_id == Conversation.business_id,
                        or_(
                            and_(Order.buyer_id == user_id, Order.seller_id == peer_id_expr),
                            and_(Order.buyer_id == peer_id_expr, Order.seller_id == user_id),
                        ),
                    )
