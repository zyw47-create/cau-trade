...
def arbitrate_refund(refund_id: int, admin_id: int, result: str, note: str) -> bool:
    ...
    with session_scope() as session:
        ...
        if result == "buyer":
            ...
            before_score = int(seller.credit_score or 100)
            next_score = max(0, before_score - 10)
            seller.credit_score = next_score
            seller.updated_at = now
            session.add(
                CreditLog(
                    user_id=seller.id,
                    change_value=next_score - before_score,
                    reason_type="refund_arbitration_buyer_win",
                    reason_detail=note or "售后仲裁买家胜诉",
                    related_type="refund",
                    related_id=str(refund_id),
            ...
            if order.item_type == "goods":
                ...
                if goods:
                    ...
                    goods.updated_at = now
            before_score = int(seller.credit_score or 100)
            next_score = min(100, before_score + 1)
            seller.credit_score = next_score
            seller.updated_at = now
            session.add(
                CreditLog(
                    user_id=seller.id,
                    change_value=next_score - before_score,
                    reason_type="refund_arbitration_seller_win",
                    reason_detail=note or "售后仲裁卖家胜诉",
                    related_type="refund",
                    related_id=str(refund_id),
                    operator_id=_nullable_admin_id(admin_id),
                    score_after=next_score,
...
def refund_evidence_detail(refund_id: int) -> dict | None:
    with session_scope() as session:
        ...
        row = session.execute(
            select(
                ...
                buyer.c.id.label("buyer_id"),
                buyer.c.nickname.label("buyer_name"),
                buyer.c.username.label("buyer_username"),
                buyer.c.credit_score.label("buyer_credit_score"),
        ...
        events = session.execute(
            select(
                ...
                OrderEvent.operator_id,
                OrderEvent.event_type,
                OrderEvent.note,
                OrderEvent.created_at,
            )
            .where(OrderEvent.order_sn == detail["order_sn"])
            .order_by(OrderEvent.created_at, OrderEvent.id)
        )
        detail["events"] = [dict(event._mapping) for event in events]
        messages = session.execute(
            text(
                ...
                SELECT
                  ...
                  status,
                  created_at
                FROM v_chat_evidence_chain
                WHERE business_type = :business_type AND business_id = :business_id
                ORDER BY created_at, message_id
                """
            ),
            {"business_type": detail["item_type"], "business_id": int(detail["item_id"] or 0)},
        ).mappings()
        evidence_messages = []
        last_hash = ""
        for msg in messages:
            item = dict(msg)
            hash_value = item.get("content_hash") or ""
            previous_hash = item.get("previous_hash") or ""
            item["hash_present"] = bool(hash_value)
            item["previous_hash_matches"] = not previous_hash or previous_hash == last_hash
            item["hash_status"] = "ok" if item["hash_present"] and item["previous_hash_matches"] else "warning"
            if hash_value:
                last_hash = hash_value
            evidence_messages.append(item)
        detail["messages"] = evidence_messages
        detail["message_count"] = len(evidence_messages)
        detail["latest_message_hash"] = last_hash
        detail["hash_chain_status"] = "ok" if all(message["hash_status"] == "ok" for message in evidence_messages) else "warning"
        detail["recalled_messages"] = [message for message in evidence_messages if message.get("status") in {"recalled", "deleted", "hidden"}]
        return detail
...
def stats_snapshot() -> dict:
    with session_scope() as session:
        ...
        goods_on_sale = int(session.scalar(select(func.count()).select_from(Goods).where(Goods.status == "on_sale")) or 0)
        today_orders = int(session.scalar(select(func.count()).select_from(Order).where(func.date(Order.created_at) == today)) or 0)
        today_amount = (
            session.scalar(
                select(func.coalesce(func.sum(Order.amount), 0)).where(
                    func.date(func.coalesce(Order.paid_at, Order.created_at)) == today,
                    Order.status.in_(["paid", "confirmed", "shipped", "completed", "refunding", "refunded"]),
                )
            )
            or 0
        )
